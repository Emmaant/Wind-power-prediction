...
#NOTE: Different normalization compared to other degrees!
def calculate_angles(x_coords, y_coords):
    num_turbines = len(x_coords)
    angles = np.zeros((num_turbines, num_turbines))
    ...
    for i in range(num_turbines):
        for j in range(num_turbines):
            if i != j:
                ...
                angle_degrees = (90 - np.degrees(angle)) % 360  # Adjust for North as 0
                angles[i, j] = angle_degrees / 360  # Normalize to range 0-1
...
def convert_adjacency_matrix_to_edge_index(A):
    ...
    # Convert result to shape (2, number of edges)
...
def create_edge_attribute(edge_index,attr_array):
    #attr_array = np.array([weightDist, normAngle])
    """
...
def get_split_index_df(df,config):
    ...
    random_idx = rng.choice(N,size=test_val_size, replace=False)
    random_index = indices[random_idx]
    #Using set difference to get train index and shuffeling
...
class Graph:
    ...
    def get_data_columns(self,config):
        ...
        print(self.data_columns_global)
    ...
    def preprocess_data(self,df,config, dir_name):
        ...
        with open('data/train_data/index_hourly.pkl', "rb") as openfile:
            index.append(pickle.load(openfile))
                    
    
        train_indices,val_indices,test_indices = index[0][0],index[0][1],index[0][2]
        ...
        # turbine_on columns remain unchanged
